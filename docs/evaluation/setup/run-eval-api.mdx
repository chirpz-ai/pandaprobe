...
All evaluation features are accessible through the REST API at `/api/v1/evaluations`. This page covers every endpoint for running trace-level and session-level evaluations.

<Note>
All endpoints require authentication via **Bearer JWT** with `X-Project-ID` header, or **API key** with `X-API-Key` and `X-Project-Name` headers.
</Note>
...
```bash
GET /api/v1/evaluations/trace-metrics
```
...
```bash
GET /api/v1/evaluations/session-metrics
```
...
```bash
POST /api/v1/evaluations/trace-runs
```
...
```bash
POST /api/v1/evaluations/trace-runs/batch
```
...
```bash
GET /api/v1/evaluations/trace-runs/{run_id}
```
...
```bash
GET /api/v1/evaluations/trace-runs?status=COMPLETED&limit=20&offset=0
```
...
```bash
GET /api/v1/evaluations/trace-runs/{run_id}/scores
```
...
```bash
POST /api/v1/evaluations/trace-runs/{run_id}/retry
```
...
```bash
DELETE /api/v1/evaluations/trace-runs/{run_id}?delete_scores=false
```
...
```bash
POST /api/v1/evaluations/trace-scores
```
...
```bash
GET /api/v1/evaluations/trace-scores
```
...
```bash
GET /api/v1/evaluations/trace-scores/{trace_id}
```
...
```bash
PATCH /api/v1/evaluations/trace-scores/{score_id}
```
...
```bash
DELETE /api/v1/evaluations/trace-scores/{score_id}
```
...
```bash
POST /api/v1/evaluations/session-runs
```
...
```bash
POST /api/v1/evaluations/session-runs/batch
```
...
```bash
GET /api/v1/evaluations/analytics/trace-scores/summary?date_from=2026-03-01T00:00:00Z
```
...
```bash
GET /api/v1/evaluations/analytics/trace-scores/trend?metric_name=task_completion&granularity=day
```
...
```bash
GET /api/v1/evaluations/analytics/trace-scores/distribution?metric_name=task_completion&buckets=10
```
...
```bash
GET /api/v1/evaluations/analytics/session-scores/history/{session_id}?metric_name=agent_reliability&limit=50
```

**Session comparison** — rank sessions by a metric (useful for finding worst-performing sessions):

```bash
GET /api/v1/evaluations/analytics/session-scores/comparison?metric_name=agent_reliability&sort_order=asc&limit=10
```
...
```bash
GET /api/v1/evaluations/trace-runs/template?metric=task_completion
```
