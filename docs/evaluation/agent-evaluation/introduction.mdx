...
In PandaProbe, a **session** represents a series of traces from a single user interaction — an entire conversation, a multi-step workflow, or an agent's lifetime. Agent evaluation aggregates per-trace signals into session-level scores that capture patterns only visible across multiple interactions.
