---
title: "Overview"
description: "Zero-code LLM tracing for OpenAI, Anthropic, and Google Gemini"
icon: "gauge"
---

Wrappers provide the simplest way to add tracing to your LLM calls. Wrap your provider client once and every API call is automatically traced — no other code changes required.

The wrapper returns the same client type with tracing injected. Your existing code continues to work identically.

All wrappers automatically capture: input messages, output messages, model name, token usage (prompt, completion, reasoning, cache tokens where available), model parameters, streaming support, time-to-first-token for streaming, and error details.
...
<Tip>
  Install only the provider extras you use so dependency resolution stays minimal in production images and lockfiles.
</Tip>
