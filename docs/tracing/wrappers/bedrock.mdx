...
<Tabs>
  ...
  <Tab title="npm">
    ```bash
    npm install pandaprobe @aws-sdk/client-bedrock-runtime
    ```
  </Tab>
  <Tab title="yarn">
    ```bash
    yarn add pandaprobe @aws-sdk/client-bedrock-runtime
    ```
  </Tab>
  <Tab title="pnpm">
    ```bash
    pnpm add pandaprobe @aws-sdk/client-bedrock-runtime
    ```
  </Tab>
</Tabs>

For Python, the `bedrock` extra installs `boto3>=1.34.0`. For async support install `aioboto3` separately — `wrap_bedrock` detects it at runtime and instruments async methods automatically without making it a hard dependency.
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    The JavaScript client uses the standard AWS credential provider chain. Set `AWS_REGION` and either export access keys, set `AWS_PROFILE`, use IAM Identity Center, or use the credentials attached to the runtime environment.
  </Tab>
</Tabs>
...
const response = await client.send(new ConverseCommand({
  ...
  messages: [{
    role: "user",
    content: [{ text: "Explain recursion in one sentence." }],
  }],
...
In TypeScript, send a `ConverseStreamCommand` and consume `response.stream` with `for await`. The wrapper preserves the AWS SDK response shape while tracing the asynchronous stream.
