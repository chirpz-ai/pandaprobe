...
<Note>
  The Python `deepagents` package requires Python 3.11 or newer. TypeScript users should also follow the engine requirements of the installed DeepAgents and LangChain packages.
</Note>
...
<Tabs>
  ...
  <Tab title="npm">
    ```bash
    npm install pandaprobe deepagents langchain @langchain/core @langchain/openai @langchain/langgraph @langchain/langgraph-checkpoint @langchain/langgraph-sdk "langsmith@>=0.7.1 <0.10.0"
    ```
  </Tab>
  <Tab title="yarn">
    ```bash
    yarn add pandaprobe deepagents langchain @langchain/core @langchain/openai @langchain/langgraph @langchain/langgraph-checkpoint @langchain/langgraph-sdk "langsmith@>=0.7.1 <0.10.0"
    ```
  </Tab>
  <Tab title="pnpm">
    ```bash
    pnpm add pandaprobe deepagents langchain @langchain/core @langchain/openai @langchain/langgraph @langchain/langgraph-checkpoint @langchain/langgraph-sdk "langsmith@>=0.7.1 <0.10.0"
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    const handler = new DeepAgentsCallbackHandler({
...
The Python example declares a `researcher` sub-agent alongside the main deep agent. When the model decides to delegate research, it calls the built-in `task` tool, which dispatches the sub-agent — the handler captures the entire nested run as one trace:
...
The Python example produces one trace where the parent agent's `task` tool span contains the entire nested `researcher` sub-agent run, including its internal `tools` chain and tool calls.
