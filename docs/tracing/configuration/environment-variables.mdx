...
```ts title="TypeScript"
import { getClient } from "pandaprobe";

// Export variables in the shell or load a .env file before first use.
const client = getClient();
```
</CodeGroup>

<Note>
  Python reads variables from `os.environ`; TypeScript reads them from `process.env`. Values are resolved at client initialization, so later environment changes have no effect. Re-run Python's `pandaprobe.init(...)` or TypeScript's `init({...})` to reconfigure.
</Note>
