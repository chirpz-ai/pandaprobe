...
<Steps>
  <Step title="Install the SDK">
    <Tabs>
      ...
      <Tab title="npm">
        ```bash
        npm install pandaprobe openai @google/genai @anthropic-ai/sdk
        ```
      </Tab>
      <Tab title="yarn">
        ```bash
        yarn add pandaprobe openai @google/genai @anthropic-ai/sdk
        ```
      </Tab>
      <Tab title="pnpm">
        ```bash
        pnpm add pandaprobe openai @google/genai @anthropic-ai/sdk
        ```
      </Tab>
    </Tabs>
  </Step>
...
<Note>
  For short-lived scripts, call Python's `pandaprobe.flush()` or TypeScript's `await flush()` before the process exits to ensure all traces are sent.
</Note>
