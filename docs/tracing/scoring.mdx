...
<Tabs>
  ...
  <Tab title="TypeScript">
    ```ts title="TypeScript"
    import { ScoreDataType, score } from "pandaprobe";

    score("your-trace-id", "user_satisfaction", "0.9", {
...
TypeScript passes the first three values positionally and uses a camelCase options object: `score(traceId, name, value, {dataType, reason, metadata})`.
...
```ts title="TypeScript"
import { score, withTrace } from "pandaprobe";
...
```ts title="TypeScript"
score(traceId, "relevance", "0.9", { dataType: ScoreDataType.NUMERIC });
score(traceId, "hallucination_free", "true", { dataType: ScoreDataType.BOOLEAN });
score(traceId, "sentiment", "positive", { dataType: ScoreDataType.CATEGORICAL });
```
