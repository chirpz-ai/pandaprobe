...
describe("queryKeys", () => {
  ...
  it("subscriptions keys are stable", () => {
    expect(queryKeys.subscriptions.current).toEqual(["subscription"]);
    expect(queryKeys.subscriptions.usage).toEqual(["subscription", "usage"]);
  });
});
