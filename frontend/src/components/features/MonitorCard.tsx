    ...
    <div className="border-engraved bg-surface p-4 space-y-3">
      <div className="flex items-start justify-between">
        <div>
          ...
          <span className="text-[10px] text-text-muted font-mono">{monitor.id.slice(0, 8)}</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs font-mono">
        ...
        <div>
          ...
          <span className="text-text">{formatRelativeTime(monitor.last_run_at)}</span>
        </div>
        <div>
          <span className="text-text-muted block">Next Run</span>
          <span className="text-text">{formatRelativeTime(monitor.next_run_at)}</span>
        </div>
      </div>

      <div className="flex gap-2 pt-1">
        {isActive ? (
          <Button variant="secondary" size="sm" onClick={() => onPause(monitor.id)}>
            <Pause className="h-3 w-3" /> Pause
          </Button>
        ) : (
          <Button variant="secondary" size="sm" onClick={() => onResume(monitor.id)}>
            <Play className="h-3 w-3" /> Resume
          </Button>
        )}
        <Button variant="secondary" size="sm" onClick={() => onTrigger(monitor.id)}>
